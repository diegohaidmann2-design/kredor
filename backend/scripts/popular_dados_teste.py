...
async def criar_dados_teste():
    ...
    for i, cliente in enumerate(clientes_criados):
        ...
        for j in range(num_emprestimos):
            valor_principal = random.choice([500, 1000, 1500, 2000, 3000, 5000, 7500, 10000])
            ...
            if metodo_calculo == 'simples':
                ...
                valor_total_com_juros = valor_principal + juros_total
            else:
                valor_total_com_juros = valor_principal * ((1 + taxa_juros_mensal / 100) ** prazo_meses)

            valor_parcela = valor_total_com_juros / prazo_meses
            juros_por_parcela = (valor_total_com_juros - valor_principal) / prazo_meses
            principal_por_parcela = valor_principal / prazo_meses
            ...
            emprestimo = {
                ...
                'valor_total_juros': round(valor_total_com_juros - valor_principal, 2),
                'status': 'ativo',
            ...
            for k in range(prazo_meses):
                ...
                if dias_atraso_calc > 0:
                    ...
                    if random.random() < 0.25:
                        status = 'parcial'
                        valor_pago = round(valor_parcela * random.uniform(0.3, 0.7), 2)
                    else:
                        status = 'pendente'
                        valor_pago = 0.0
                else:
                ...
                if status == 'atrasado' and dias_atraso_calc > 0:
                    valor_multa = round(valor_parcela * 0.02, 2)  # 2% multa
                    valor_juros_mora = round(valor_parcela * 0.01 * (dias_atraso_calc / 30), 2)  # 1%/mes
                ...
                parcela = {
                    ...
                    'valor_principal': round(principal_por_parcela, 2),
                    'valor_juros': round(juros_por_parcela, 2),
                    'valor_total': round(valor_parcela, 2),
                    'valor_pago': valor_pago,
                    'valor_multa': valor_multa,
                    'valor_juros_mora': valor_juros_mora,
                    'saldo_devedor': round(valor_principal * (1 - (k + 1) / prazo_meses), 2),
    ...
    total_emp = sum(e['valor_principal'] for e in emprestimos_criados)
    total_recebido = sum(p['valor_pago'] for p in pagamentos_criados)
